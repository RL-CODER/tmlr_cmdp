...
class BAGK:
    def __init__(self, cmdp, T, m, adv_data=None, delta=0.01):
        ...
        self.theta = (self.D + 0.5) / (3 * np.sqrt(self.T) * (1 + self.D)**2)
        self.zeta = 0.0
    ...
    def validate_omega(self, verbose=True):
        ...
        if verbose:
            ...
            print('-'*60)
            for i in range(min(10, self.A_eq.shape[0])): # Show first 10 rows
    ...
    def run(self):
    	...
    	for t in range(1, self.T + 1):
    	    ...
    	    for h in range(self.H):
    	        ...
    	        loss_val = ell_t_full[self.index_map[(state, action, layer_idx)]]
    	        constraint_val = c_t_full[self.index_map[(state, action, layer_idx)]]
    	    ...
    	    for visit in trajectory:
    	        ...
    	        a = visit['action']
    	        h = visit['layer']
    	        idx = self.index_map[(s, a, h)]
    	        
    	        rho_sa = self.rho_vector[idx]
    	        
    	        hat_ell_t[idx] = visit['loss'] / (rho_sa + self.Lambda_t)
    	        hat_c_t[idx] = visit['constraint'] / (rho_sa + self.Lambda_t)
...
def compute_theoretical_bounds_BAGK(t_vals, SHA, T):
    regret_bound = 2 * SHA * (1 + (np.sqrt(6 * t_vals) / 2) + (np.sqrt(12 * t_vals) / 2))
    D = np.sqrt(SHA)
    gamma = 1 - 1 / np.sqrt(t_vals)
    num = 1 + (np.sqrt(6 * t_vals) / 2) + (np.sqrt(12 * t_vals) / 2)
    common = (12 * np.sqrt(t_vals * SHA) * (1 + np.sqrt(SHA))**2 / (2 * np.sqrt(SHA) + 1))
    log_term = np.log(num)
    mask = (gamma != 0)
    if np.any(mask):
        log_term[mask] += np.log(1 / gamma[mask])
    violation_bound = common * log_term
...
def main():
    ...
    save_name = f"BAG-K_S={S}_A={A}_H={H}_T={T}.png"
    ...
    for seed in seeds:
        ...
        bag_k = BAGK(cmdp, T, m, adv_data, delta)
       
        bag_k.create_omega()
        bag_k.validate_omega(verbose=False)
       
        cum_regret, cum_viol = bag_k.run()
    ...
    if regrets and viols:
        ...
        SHA = S * H * A
        theo_regret, theo_viol = compute_theoretical_bounds_BAGK(t_vals, SHA, T)
       
        plt.figure(figsize=(14, 6))
        plt.subplot(1, 2, 1)
        plt.plot(t_vals, regret_mean, color='blue', label='BAG-K Empirical Regret',
                 linestyle='dashed', linewidth=3.0)
        if num_seeds > 1:
            plt.fill_between(t_vals, regret_lower, regret_upper, alpha=0.3, color='blue')
        plt.plot(t_vals, theo_regret, color='red', label='BAG-K Theoretical Regret',
        ...
        plt.title(f'BAG-K Regret Plot: S={S}, A={A}, H={H}', fontsize=15)
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=10)
       
        plt.subplot(1, 2, 2)
        plt.plot(t_vals, viol_mean, color='blue', label='BAG-K Empirical Violation',
                 linestyle='dashed', linewidth=3.0)
        if num_seeds > 1:
            plt.fill_between(t_vals, viol_lower, viol_upper, alpha=0.3, color='blue')
        plt.plot(t_vals, theo_viol, color='red', label='BAG-K Theoretical Violation',
                 linestyle='solid', linewidth=3.0)
        plt.xlabel('Number of Episodes (t)', fontsize=12)
        plt.ylabel('Cumulative Violation $\mathcal{Z}_{t}$', fontsize=12)
        plt.title(f'BAG-K Violation Plot: S={S}, A={A}, H={H}', fontsize=15)
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=10)
